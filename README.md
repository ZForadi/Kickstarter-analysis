...
The second analysis, Outcomes Based on Goals, will utilize the `COUNTIFS()` function in excel, to observe the outcomes for campaigns in the Sub Category, Plays, based on the campaigns' goal. Specifically, it will show the count and percentage of play campaigns that were "Successful," "Failed," and "Canceled" by categorizing the play campaigns into 12 different goal-amount ranges. The goal-amount ranges are defined in the Analysis of Outcomes Based on Goals section below. The purpose of this analysis is to see which goal-amount range(s) had the:
...
* Parent Category - the category of campaign. Columnn Q in the Kickstarter tab. 
* Subcategory - the subcategory of the campaign. Column R in the Kickstart tab. 
...
* Year - the year the campaign launched. Column U in the Kickstarter tab. 
...
4. The last criteria was based on the subcategory of the campaign and was written as: `Kickstarter!$R:$R, "plays"`. The first part of the criteria denotes we want to use the subcategory column in the Kickstarter Tab. The second part states we only want to use the "plays" subcategory. This part of the function remained the same for all outcomes and all goal-amount ranges. "Plays" is the same subcategory that Louise's campaign falls under. This ensures we are looking at the outcomes of campaigns that are most relevant to Louise's campaign.

Once B2:D13 was populated with the functions noted above, I then used the `SUM()` function in excel to sum the count of successful campaigns, failed campaigns and canceled campaigns in each goal-amount range row. This was populated under the"Total Projects" column. 

Finally, to calculate the percentage of successful, failed and canceled campaigns in each goal-amount range, I divided the count of the respective outcome by the total projects for each goal-amount range. I then formatted the number into a percentage. 
...
In looking at the the Theater_Outcomes_vs_Launch graph above it can be concluded that across all months, the number of successful theater campaigns is higher than failed and canceled campaigns. In general, theater campaigns have been mostly succssful when launched in any month of the year. 

Theater campaigns that were launched in May had the highest amount of successful campaigns, while theater campaigns that were launched in December had the lowest amount of successful campaigns. There is a rise in the number of successful campaigns for campaigns that were launched from March to May (spring time). For campaigns that were launched after May all the way to September, the number of successful campaigns fall. The number of successful campaigns rises slightly in October, and then continues to decline for launches in November and December. 

Campaigns launched in May also had the highest amount of failed campaigns, and campaigns that launched in July and Octobera had the second highest amount of failed campaigns. November had the least amount of failed campaigns (with December, January and March also having lower occurences of failed campaigns).
...
In looking at the the Outcomes_vs_Goals graph above, it can be concluded that plays with goals that are less than $4,999 are successful. Goals less than $1,000 were most successful with a 76% success rate and goals which ranged $1000 to $4999 had a similar success rate (73% success rate). Most projects were in the $1000 to $4999 goal-amount range (total of 534 projects), and goals less than $1,000 had the second highest amount of projects (186). Play campaigns that had goals between $35,000 to $39,999 and $40,000 to $44,999 both had a 67% rate with total projects of 6 and 3 respectively. 
...
In general, it can be concluded that play campaigns that have a lower goal amount (less than $1,000 to $4,999) are most successful, while play campaigns that have goals of $45,000 or higher mostly fail. 
...
Additionally, the dataset accounts for campaigns in 21 different countries. 
